...
def main(args):
    ...
    ax_2A = letter('A',plot_value_by_iteration)(f, gs[0,0], transform_acc(acc[:,selected_steps_idx]), scales, selected_steps, ylabel = 'accuracy', clabel = 'gain $G$', ylim = (-0.1, 1.1), cticks = np.arange(1,6), threshold = 0.95, chance = 1/19)
    ax_3C = letter('B',violinplot)(f, gs[0,1], [best_e[patient_data['Group']=='MD'], best_e[patient_data['Group']=='TD']] , names = ['MLD\npDNN','TD\npDNN'], xlabel = '', ylabel = f'gain $G$', ylim = (0.5, 5.5), c = [c_md, c_td], pd = 'all')
    ax_5A = letter('C', barplot_by_layer)(f, gs[1,0], modules, [addsub_rs_best_e_md, addsub_rs_best_e_td], c = [c_md, c_td], label = ['MLD', 'TD'], ylabel = 'add-sub NRS')
    ax_5B = letter('D', scatter)(f, gs[1,1], addsub_rs_child, addsub_rs_best_e[-1], xlabel = 'children add-sub NRS', ylabel = 'pDNN add-sub NRS', correlate = True)
    ax_6E = letter('E',violinplot)(f, gs[2,0], [trueness_response_best_e[patient_data['Group']=='MD', best_step_b_idx], trueness_response_best_e[patient_data['Group']=='TD', best_step_b_idx]] , names = ['MLD\npDNN','TD\npDNN'], ylim = (-0.1, 1.1), c = [c_md, c_td], pd = 'all', ylabel = 'numerical systematic error')
    ax_6G = letter('F',violinplot)(f, gs[2,1], [h_b_best_e[patient_data['Group']=='MD', best_step_b_idx], h_b_best_e[patient_data['Group']=='TD', best_step_b_idx]] , names = ['MLD\npDNN','TD\npDNN'], ylim = (-1, 20), c = [c_md, c_td], pd = 'all', ylabel = '# different responses')
    ax_7F = letter('G', barplot_by_layer)(f, gs[3,0], modules, [correlation_best_e_md[:,modules_idx], correlation_best_e_td[:,modules_idx]], c = [c_md, c_td], label = ['MLD', 'TD'], ylabel = 'inter-manifold correlation')
    ax_8B = letter('H',violinplot)(f, gs[3,1], [additional_training[patient_data['Group']=='MD'], additional_training[patient_data['Group']=='TD']] , names = ['MLD','TD'], ylim = (-10, 150), c = [c_md, c_td], pd = [(0,1)], ylabel = 'additional training (%)')#, title = None)
    f.savefig(f'{figure_path}/png/figureS9.png', dpi = 1200)
    f.savefig(f'{figure_path}/pdf/figureS9.pdf', dpi = 1200)
